...
- **Task-book pre-flight for new public symbols (hard rule · GM-16 · CB-P5-28 承接)**: before dispatching a subagent, if the task will **create a new public symbol** (class / dataclass / router / middleware / service / dependency / module-level exported function), the Lead MUST first call `codegraph_explore` on that symbol name to confirm it does not already exist — or, if it exists, that the differences are reconcilable and the task book explicitly documents them. GM-11 covers the subagent-side pre-flight (Alembic head); GM-16 covers the Lead-side task-book pre-flight. Exempt: pure constant flips (e.g. GM-22 default reversal), pure test additions, docstring/comment edits. Not exempt: any new class / router / middleware / service — even when framed as "skeleton" or "minimal". Skipping this check produced a "task book blindly assumes greenfield" failure mode where the subagent hits a stopped-early GM-10 report; the correct fix is Lead pre-flight, not subagent apology.
