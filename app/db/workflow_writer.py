"""`app.db.workflow_writer` — 数据 PR-8 WorkflowDefinition DB 主写路径 · PR-22 反转默认。

**数据 PR-22**（Wave 3-N.5 主线 B）反转默认后：`WORKFLOW_DEFINITION_PRIMARY_WRITE`
未设 / 空 env → `"db"`（既往为 `"json"`）；本模块变为**默认 DB 主写**的落盘承接点。
只有显式 `WORKFLOW_DEFINITION_PRIMARY_WRITE=json` 时，`workflow_store.save_runninghub_workflow_store`
...
立即生效；此时 `workflow_store` 走 legacy `main.save_runninghub_workflow_store` JSON
...
- [[40 实施计划/数据模型治理实施计划与PR清单]] PR-8 / PR-22
